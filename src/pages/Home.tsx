  ...
  return (
    <div className="flex flex-col w-full text-foreground bg-background font-sans animate-in fade-in duration-500">
          <section id="hero" className="relative w-full min-h-[65vh] flex flex-col items-center justify-center bg-background text-foreground transition-colors duration-300 pt-24 sm:pt-36 pb-24 sm:pb-36">
            ...
            <div className="w-full absolute bottom-0 inset-x-0 flex justify-center pointer-events-none z-10 translate-y-[1px]">
              <CitySkylineGraphic className="w-full h-20 sm:h-28 md:h-36 lg:h-44 xl:h-48 2xl:h-52 text-foreground/80 dark:text-foreground/75" />
            </div>
          </section>
          ...
          <div ref={projectsRef} className="space-y-32">
            {loadingProjects ? (
              ...
              featuredProjects.map((project, index) => {
                const image = (project.parsedImageUrls && project.parsedImageUrls.length > 0) ? project.parsedImageUrls[0] : DEFAULT_IMAGES.service1;
                const isEven = index % 2 === 1;
