  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        ...
        // ⚡ Bolt: Pre-parse JSON during data fetch instead of inline render loops
        // Impact: Reduces time complexity from O(N^2) to O(N) when list is rendered,
        // mitigating main thread blocking from heavy string parsing on frequent re-renders.
        const enrichedData = Array.isArray(data) ? data.map((project: any) => {
          let thumbnail = 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?q=80&w=2070&auto=format&fit=crop';
          try {
            const urls = JSON.parse(project.imageUrls || '[]');
                ...
                >
                  <div className="relative w-full h-full flex flex-col sm:flex-row items-stretch overflow-hidden">
                    ...
                    <div className="relative w-full sm:w-[62%] h-[240px] sm:h-full overflow-hidden flex-shrink-0">
                      <img 
                        src={bigProjects[activeBigIndex].thumbnail}
                  ...
                  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-30 flex items-center gap-1.5">
                    {bigProjects.map((_, idx) => (
                      <button
                        ...
                        aria-label={language === 'ar' ? `المشروع ${idx + 1}` : `Project ${idx + 1}`}
                      />
                    ))}
                  </div>
                </>
