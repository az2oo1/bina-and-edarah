...
interface Project {
  ...
  parsedImages?: string[]; // Pre-parsed images to avoid re-parsing in render
}
...
export default function Projects() {
    ...
    const fetchProjects = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          // Pre-parse JSON to avoid O(n^2) re-renders
          const parsedData = data.map((p: any) => {
            let parsedImages: string[] = [];
            try {
              if (p.imageUrls) {
                parsedImages = JSON.parse(p.imageUrls);
              }
            } catch (_) {}
  ...
  const getFirstImage = (parsedImages?: string[]) => {
                ...
                >
                  <div className="relative w-full h-full flex flex-col sm:flex-row items-stretch overflow-hidden">
                    ...
                    <div className="relative w-full sm:w-[62%] h-[240px] sm:h-full overflow-hidden flex-shrink-0">
                      <img 
                        src={getFirstImage(bigProjects[activeBigIndex].parsedImages)}
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="lg:w-[45%] h-[220px] sm:h-[280px] overflow-hidden bg-muted relative border-b lg:border-b-0 border-border/40">
                          <img 
                            src={getFirstImage(project.parsedImages)}
