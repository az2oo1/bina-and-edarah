      ...
      try {
        ...
        if (Array.isArray(data)) {
          formattedData = data.map((p: any) => {
            let parsedFirstImage = 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?q=80&w=2070&auto=format&fit=crop';
            // ⚡ Bolt: Pre-parse JSON during data fetch instead of inside the render loop
            // to prevent O(N) synchronous parsing bottlenecks on every component re-render.
            try {
              const urls = JSON.parse(p.imageUrls || '[]');
              if (urls.length > 0) {
                parsedFirstImage = urls[0];
              }
            } catch (err) {}
            return {
              ...p,
                ...
                >
                  <div className="relative w-full h-full flex flex-col sm:flex-row items-stretch overflow-hidden">
                    ...
                    <div className="relative w-full sm:w-[62%] h-[240px] sm:h-full overflow-hidden flex-shrink-0">
                      <img 
                        src={bigProjects[activeBigIndex].parsedFirstImage}
