...
## 2026-07-22 - Authentication Bypass Vulnerability (Hardcoded Backdoor)
**Vulnerability:** A hardcoded authentication fallback block in `/api/login` permitted login with `username === 'admin' && password === 'admin'` *after* the database check, allowing an attacker to bypass the actual admin password if the admin account password was changed in the database.
**Learning:** Hardcoded credentials should never be evaluated as fallbacks in production environments, especially after an attempted database check, as they provide a permanent backdoor that bypasses legitimate database credential rotation or changes.
**Prevention:** Never include hardcoded credentials in authentication logic for "preview" or "fallback" modes in production systems. Instead, properly seed administrative accounts into the database upon initialization and rely solely on those verified database records.
