...
## 2026-07-31 - Hardcoded Authentication Bypass and Default Secret
**Vulnerability:** A hardcoded `JWT_SECRET` fallback and a backdoor for `admin:admin` unconditionally bypassed the database, allowing unauthorized access if secrets weren't properly injected or the DB failed.
**Learning:** Fallback security mechanisms that rely on hardcoded secrets are a critical security failure, as they are trivial to guess or find in source code. Fallbacks should fail-closed.
**Prevention:** Never provide a default fallback for security-critical secrets like JWT keys. Missing required configuration should cause the server to fail to start (e.g. by throwing an Error). Remove development-only backdoors (like hardcoded admin logins) in production code entirely, or isolate them strongly.
