...
## 2026-07-24 - SQL Injection in Dynamic Settings Update
**Vulnerability:** The `updateGlobalSettings` function used `prisma.$executeRawUnsafe` with manually constructed SQL strings where the `field` variable, sourced directly from user-controlled object keys (`Object.keys(data)`), was concatenated without parameterization.
**Learning:** Even fallback logic written when the ORM fails must be rigorously secure. The combination of dynamic column updates, manual string escaping, and untrusted object keys created a critical SQL injection point that bypassed Prisma's safe abstractions.
**Prevention:** Always validate dynamic field names against a strict hardcoded allowlist before building queries. For fallback raw SQL involving dynamic columns, use `Prisma.sql` for the overall query, and securely inject the allowlisted dynamic column name using `${Prisma.raw(field)}` to ensure parameterization of the values while permitting variable columns.
